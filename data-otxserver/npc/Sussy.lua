...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "addon") or MsgContains(message, "outfit") or MsgContains(message, "hat") then
		local addonProgress = player:getStorageValue(DarkKonia.OutfitQuest.Citizen.AddonHat)
	...
	if npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, 'yes') then
			...
			player:setStorageValue(DarkKonia.OutfitQuest.Citizen.AddonHat, 1)
			player:setStorageValue(DarkKonia.OutfitQuest.Citizen.MissionHat, 1)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, 'yes') then
			...
			elseif player:getItemCount(5902) < 50 then
				...
				player:setStorageValue(DarkKonia.OutfitQuest.Citizen.MissionHat, 0)
				player:setStorageValue(DarkKonia.OutfitQuest.Citizen.AddonHat, 2)
			end
		else
