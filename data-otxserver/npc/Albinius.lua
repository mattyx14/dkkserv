...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 1 then
		if (player:getStorageValue(DarkKonia.ForgottenKnowledge.Tomes) == 1) then
	...
	if MsgContains(message, 'tomes') and player:getStorageValue(DarkKonia.ForgottenKnowledge.Tomes) < 1 then
		npcHandler:say({
