function onUse(cid, item, frompos, item2, topos)
        ...
        if item.uid == 10005 then -- the unique id of chest
		queststatus = getPlayerStorageValue(cid,10005) --storage value, keep at chests unique id
		if queststatus == -1 then -- checks if quest is done
			...
			setPlayerStorageValue(cid,10005,1) -- sets storage value up 1
		else
