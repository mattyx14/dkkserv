function onUse(cid, item, frompos, item2, topos)
	if item.uid == 50004 then
		queststatus = getPlayerStorageValue(cid, 50004)
		if queststatus == -1 or queststatus == 0 then
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You have found a Beggar outfit.")
			item_uid = doPlayerAddItem(cid, 2503, 1)
			item_uid = doPlayerAddItem(cid, 7939, 1)
			item_uid = doPlayerAddItem(cid, 2504, 1)
			setPlayerStorageValue(cid, 50004, 1)
		else
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "It\'s empty.")
		end
	else
		return false
	end

	return true
end
