...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=185, y=385, z=7}
      bottonright={x=203, y=403, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
        ...
        if species == 1 then
      checkforsummon = doSummonCreature("Pirate Buccaneer",checking)
        elseif species == 2 then
      checkforsummon = doSummonCreature("Pirate Corsair",checking)
        elseif species == 3 then
      checkforsummon = doSummonCreature("Pirate Cutthroat",checking)
        elseif species == 4 then
      checkforsummon = doSummonCreature("The Brutus Bloodbeard",checking)
        elseif species == 5 then
      checkforsummon = doSummonCreature("Pirate Marauder",checking)
        elseif species == 6 then
      checkforsummon = doSummonCreature("The Ron the Ripper",checking)
        elseif species == 7 then
      checkforsummon = doSummonCreature("Pirate Ghost",checking)
        else
      checkforsummon = doSummonCreature("Smuggler",checking)
        end

        -- if species<5 and species >=0 then
      -- checkforsummon=doSummonCreature("Lethal Lissy",checking)
        -- elseif species == 2 then
      -- checkforsummon=doSummonCreature("The Ron the Ripper",checking)
        -- end
    ...
    if checking.x>bottonright.x then
      ...
      --doPlayerSay(cid,"Pirates are launching an attack on Rook Island (Boss)!",23)
