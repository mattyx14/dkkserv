...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=220, y=239, z=7}
      bottonright={x=256, y=255, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
        ...
        if species == 1 then
      checkforsummon = doSummonCreature("Kongra",checking)
        elseif species == 2 then
      checkforsummon = doSummonCreature("Sibang",checking)
        elseif species == 3 then
      checkforsummon = doSummonCreature("Merlkin",checking)
    ...
    if checking.x>bottonright.x then
      ...
      --doPlayerSay(cid,"Ape's on Mer Jungle!",23)
