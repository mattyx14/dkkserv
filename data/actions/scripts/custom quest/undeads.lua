...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=352, y=106, z=7}
      bottonright={x=412, y=154, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
        ...
        if species == 1 then
      checkforsummon = doSummonCreature("Phantasm",checking)
        elseif species == 2 then
      checkforsummon = doSummonCreature("Spectre",checking)
        elseif species == 3 then
      checkforsummon = doSummonCreature("Betrayed Wraith",checking)
      ...
      checkforsummon = doSummonCreature("Blightwalker",checking)
        end

        -- if species<5 and species >=0 then
      -- checkforsummon=doSummonCreature("bones",checking)
        -- elseif species == 5 then
      -- checkforsummon=doSummonCreature("dracola",checking)
        -- end
    ...
    if checking.x>bottonright.x then
      ...
      --doPlayerSay(cid,"A black magic made undeads to come back in our desert of Anshara!",23)
