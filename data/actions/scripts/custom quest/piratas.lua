...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=580, y=412, z=7}
      bottonright={x=625, y=457, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
      ...
      checkforsummon = doSummonCreature("Pirate Skeleton",checking)
      ...
      -- checkforsummon=doSummonCreature("Pirate Marauder",checking)
        -- elseif species == 2 then
      -- checkforsummon=doSummonCreature("Quara Hydromancer",checking)
        -- end
    ...
    if checking.x>bottonright.x then
      ...
      --doPlayerSay(cid,"Pirates are launching an attack on Mer Jungle Island!",23)
