...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=118, y=38, z=7}
      bottonright={x=184, y=66, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
      ...
      checkforsummon = doSummonCreature("the obliverator",checking)
        elseif species == 6 then
      checkforsummon = doSummonCreature("Diabolic Imp",checking)
        elseif species == 7 then
      checkforsummon = doSummonCreature("Juggernaut",checking)
        else
      checkforsummon = doSummonCreature("Fury",checking)
        end

        -- if species<5 and species >=0 then
      -- checkforsummon=doSummonCreature("apocalypse",checking)
