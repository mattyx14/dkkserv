...
function onUse(cid, item, frompos, item2, topos)
  if getPlayerAccess(cid) ~=0 then
      topleft={x=646, y=323, z=7}
      bottonright={x=810, y=471, z=7}
      maxmonsters= nil
    ...
    if spawnrate==0 and summonedtotal ~= maxmonsters then
        ...
        if species == 1 then
      checkforsummon = doSummonCreature("centipede",checking)
        elseif species == 2 then
      checkforsummon = doSummonCreature("larva",checking)
        elseif species == 3 then
      checkforsummon = doSummonCreature("scarab",checking)
        elseif species == 4 then
      checkforsummon = doSummonCreature("ancient scarab",checking)
    ...
    if checking.x>bottonright.x then
      ...
      --doPlayerSay(cid,"Anshara is under attack!",23)
