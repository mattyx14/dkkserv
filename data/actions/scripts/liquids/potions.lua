...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif itemId == manaPot then
		if not doTargetCombatMana(0, target, 125, 175, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == strongHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 400, 500, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == strongManaPot then
		...
		if not doTargetCombatMana(0, target, 185, 250, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatSpiritPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 400, 550, CONST_ME_MAGIC_BLUE) or not doTargetCombatMana(0, target, 250, 350, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatManaPot then
		...
		if not doTargetCombatMana(0, target, 250, 400, CONST_ME_MAGIC_BLUE) then
			return false
		end
		player:addCondition(exhaust)
		target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
	...
	elseif itemId == ultimateHealthPot then
		...
		if not doTargetCombatHealth(0, target, COMBAT_HEALING, 850, 1200, CONST_ME_MAGIC_BLUE) then
			return false
		end
