	...
	if item.itemid == config.leverId then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(config.timer) > os.time() then
					...
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You or a member in your team have to wait ".. config.timeToFightAgain .."  hours to face Jaul again!")
					return true
				end
		...
		for i = 1, #specs do
			...
			if spec:isPlayer() then
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "There's someone fighting with Jaul.")
				return true
			end
			spec:remove()
		end
