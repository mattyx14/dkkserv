local config = {
	...
	timer = DarkKonia.OutfitQuest.ZarabustorTimmer,
	blockLever = true,
	value = 1,
	centerRoom = Position(853, 1046, 10),
	playerPositions = {
		Position(868, 1024, 11),
		Position(867, 1024, 11),
		Position(869, 1024, 11)
	},
	teleportPosition = Position(853, 1050, 10),
	bossPosition = Position(853, 1041, 10),
	kickPos = Position(1021, 189, 7)
}
...
function jaulLever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == config.leverId then
		...
		local specs, spec = Game.getSpectators(config.centerRoom, false, false, 10, 10, 5, 5)
		for i = 1, #specs do
			spec = specs[i]
