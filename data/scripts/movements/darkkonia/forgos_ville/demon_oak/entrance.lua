...
function entrance.onStepIn(creature, item, position, fromPosition)
	...
	if player:getStorageValue(DarkKonia.DemonOak.Done) >= 1 then
	...
	if (#Game.getSpectators(DEMON_OAK_POSITION, false, true, 9, 9, 6, 6) == 0) then
			if (player:getItemCount(9388) == 0) then
				if player:getStorageValue(DarkKonia.DemonOak.Progress) < 1 then
		...
		player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
		player:setStorageValue(DarkKonia.DemonOak.Progress, 1)
