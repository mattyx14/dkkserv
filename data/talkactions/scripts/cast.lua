...
function onSay(cid, words, param, channel)
	if(exhaustion.check(cid, config.storage)) then
		doPlayerSendCancel(cid, "You can cast only once per " .. config.exhaustion .. " minutes.")
		return true
	end
	...
	elseif isInArray({"desc", "description", "d"}, tmp[1]) then
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Cast password was set to: ")
